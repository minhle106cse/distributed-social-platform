...
- **CÒN LẠI:** (a) tạo GitHub repo `distributed-social-platform_search-service` → init/push/`git submodule add` → commit toàn bộ C1+C2 (code + `prisma.service` fix đang ở working tree, chưa versioned); (b) **C3** (Claude summarization + circuit breaker).
