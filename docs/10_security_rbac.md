...
- **API Check-in (Emotion):** Tối đa `5 request / 1 phút` cho mỗi User. (Tránh cày EXP ảo).
...
- **Khóa AI Agent:** AI (System Runtime Agent) hoàn toàn **Read-only** đối với CSDL chính. Mọi thao tác nó muốn làm (Tặng đồ, gửi thông báo) phải qua một gRPC call nội bộ có token nội bộ cực kỳ khắt khe, không được quyền `UPDATE` trạng thái bảng `Wallet` hay `PlantGuardian` trực tiếp bằng SQL.

## 4. Phân quyền Khu Phố (Neighborhood RBAC)
- **MAYOR:** Đặc quyền Kick (Evict) thành viên, gửi Thư mời định cư, và đổi tên Khu phố. Quyền này tự động bị thu hồi nếu Thị trưởng offline quá 14 ngày (Auto-Transfer).
- **DEPUTY_MAYOR:** Có quyền gửi Thư mời định cư. Là ưu tiên kế nhiệm đầu tiên khi Auto-Transfer.
- **CITIZEN:** Quyền chỉ đọc + tưới hộ + tặng quà + đóng góp năng lượng công trình.
- Backend BUỘC XÁC THỰC role: mọi API của Neighborhood phải check `NeighborhoodMember.role` trước khi xử lý, cấm hardcode hay chỉ dựa vào Frontend validation.
