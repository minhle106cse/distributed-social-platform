...
> ✅ **TRẠNG THÁI: LIVE ở notification-service (Milestone B2, 2026-06-30).** `NotificationEventsConsumer` (group `notification-service-knowledge-group`) subscribe **cả** `knowledge-events` + `engagement-events` → 1 `EventRouter` register 3 handlers: `ItemPublishedHandler` (fan-out NEW_IN_SPACE to space followers, exclude author) + `FollowCreatedHandler` (upsert `space_followers` projection) + `FollowRemovedHandler` (delete from projection). `space_followers` = local projection trong `notification_db`, KHÔNG join `core_db`. FollowCreated/Removed từ `core-api/engagement` (`EngagementModule` import `OutboxModule`; follow/unfollow handlers append event qua `OUTBOX_REPOSITORY`, `transactional:true`). B1 pattern: Idempotent via `@@unique([recipientUserId, sourceEventId])` + `createMany skipDuplicates`. Poison pill → log + commit (skip). Handler error → log + rethrow (no commit, retry on restart). DLQ bounded retry → TODO.
