...
> ⛔ **TRẠNG THÁI: chưa triển khai (deferred tới read phase, Phase 3).** Phần consumer + read-model projection (worker feed, EventRouter, DLQ, ProcessedEvent) **đã bị gỡ 2026-06-30** vì làm sớm khi chưa có đường đọc. Mục dưới là **thiết kế tham chiếu** cho khi dựng consumer thật — KHÔNG mô tả code hiện có. Producer backbone (§4.1) thì LIVE. Quy tắc: read model/projection chỉ dựng khi read phase tới, và phải nêu thành quyết định để duyệt.
